...
export function TabBar(): JSX.Element {
  ...
  const handleNewTab = (): void => {
    ...
    const targetConnectionId = current?.connectionId ?? activeConnectionId
    const activeConnection = targetConnectionId
      ? connections.find((item) => item.id === targetConnectionId)
      : null
    ...
    const id = newTab(targetConnectionId ?? undefined)
    if (current?.selectedDatabase)
      useQueryStore.getState().updateTabDatabase(id, current.selectedDatabase)
  }
  ...
  const confirmCloseTabs = (targetTabs: QueryTab[]): boolean => {
    ...
    if (targetTabs.some((tab) => tab.isLoading)) {
      alert('部分标签正在执行，请先停止执行')
      return false
    }
        ...
        {tabs.map((tab) => (
          ...
          >
            ...
            {tab.hasPendingChanges && !tab.isLoading && (
              <span
                className="w-2 h-2 rounded-full bg-accent-yellow shrink-0"
                title="有未提交的修改"
              />
            )}
